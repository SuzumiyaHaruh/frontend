...
export default {
  ...
  methods: {
    availableTime: function () {
      ...
      const maxMonth = ("0" + (now.getMonth() + 3)).slice(-2);
      const hour = ("0" + (now.getHours())).slice(-2);
      const min = ("0" + (now.getMinutes())).slice(-2);
      const minRange = now.getFullYear() + "-" + currentMonth + "-" + day + "T" + hour + ":" + min;
      const maxRange = now.getFullYear() + "-" + maxMonth + "-" + day + "T" + hour + ":" + min;
    ...
    checkEmptyErr: function () {
      for (const typeErr in this.errorObj) {
